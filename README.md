...
No pre-built image is published yet, so `docker-compose.yml`'s `build:` context needs the rest of the repo (`src/parser.py`, `src/parser_schema.py`, and `src/structs.py`) alongside it — copying just the compose file will not build. Clone the whole repository onto your Docker host instead:
