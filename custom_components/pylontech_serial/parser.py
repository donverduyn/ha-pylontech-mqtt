...
class PylontechParser:
        ...
        for line in lines:
            ...
            if parts and parts[0] == "Power" and "Coulomb" in parts:
                ...
                _LOGGER.debug(
                    ...
                    temp_low_idx, temp_high_idx, volt_low_idx, volt_high_idx,
                    status_idx, volt_st_idx, curr_st_idx, temp_st_idx, soc_idx,
                )
                break
    ...
    def parse_info(raw_text: str, system: PylontechSystem) -> PylontechSystem:
        ...
        for line in lines:
            ...
            if "device name" in key:     system.model         = val
            if "main soft" in key:       system.fw_version    = val
            if "board version" in key:   system.board_version = val
            if key == "soft version":    system.soft_version  = val
            if key == "boot version":    system.boot_version  = val
            if "comm version" in key:    system.comm_version  = val
            if "release date" in key:    system.release_date  = val
            if "barcode" in key:         system.barcode       = val
            if "specification" in key:   system.spec          = val
            if "cell number" in key:
                try: system.cell_count = int(val)
                except: pass
            if "max dischg curr" in key:
                try: system.max_dischg_curr = abs(int(re.sub(r'[^\d-]', '', val))) / 1000.0
                except: pass
            if "max charge curr" in key:
                try: system.max_charge_curr = int(re.sub(r'\D', '', val)) / 1000.0
                except: pass
